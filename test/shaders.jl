...
@testset "Shader cache" begin
  @test_throws "`Device` expected" @fragment "hello" test_shader(::Mutable{Vec4}::Output)

  vert_shader = @vertex device test_shader(::Mutable{Vec4}::Output)
  @test isa(vert_shader, Shader)

  frag_shader = @fragment device test_shader(::Mutable{Vec4}::Output)
  ...
  sh = @fragment device test_shader(::Mutable{Vec4}::Output)
  ...
  sh = @fragment device cached = false test_shader(::Mutable{Vec4}::Output)
  @test cache.shaders.diagnostics.hits == 1
  @test cache.shaders.diagnostics.misses == 1
  @test length(cache.shaders) == 1
  sh = @fragment device test_shader(::Mutable{Vec{4,Float64}}::Output)
  @test cache.shaders.diagnostics.hits == 1
